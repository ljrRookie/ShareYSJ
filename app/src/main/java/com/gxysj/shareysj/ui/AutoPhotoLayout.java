...
public class AutoPhotoLayout extends LinearLayout {
    ...
    private YSJDelegate mDelegate = null;
    private ImageView mTargetImageVew = null;
    ...
    private IconTextView mIconRemove;
    private int mDel;
    private WeakHashMap<Integer, String> mImageFileWeakHashMap;
    ...
    public AutoPhotoLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mImageFileWeakHashMap.put(0,"");
        mImageFileWeakHashMap.put(1,"");
        mImageFileWeakHashMap.put(2,"");
        ...
        mImageMargin = typedArray.getInt(R.styleable.camera_flow_layout_item_margin, 8);
        mIconSize = typedArray.getDimension(R.styleable.camera_flow_layout_icon_size, 20);
        typedArray.recycle();
    }

    public final void setDelegate(YSJDelegate delegate) {
    ...
    private void initAddIcon() {
        ...
        mIconAdd.setBackgroundResource(R.drawable.edit_white_bg);
      ...
      /*  mIconRemove = new IconTextView(getContext());
        mIconRemove.setText("{fa-minus}");
        mIconRemove.setGravity(Gravity.CENTER);
        mIconRemove.setTextSize(mIconSize);
        mIconRemove.setBackgroundResource(R.drawable.edit_white_bg);
        mIconRemove.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                RxToast.normal("删除");
                final ImageView delegateImageView = (ImageView) findViewById(mDel);
                //设置图片逐渐消失的动画
                ...
                delegateImageView.setAnimation(alphaAnimation);
                alphaAnimation.start();
                AutoPhotoLayout.this.removeView(delegateImageView);
                mCurrentNum -= 1;
        ...
        this.addView(mIconRemove);
        mIconRemove.setVisibility(GONE);*/
    ...
    public final void onCropTarget(Uri uri) {
        ...
        Glide.with(mDelegate)
                .load(uri)
                .centerCrop()
                .transform(new GlideRoundTransform(mDelegate.getContext(),8))
                .into(mTargetImageVew);
        int id = mTargetImageVew.getId();
        String path = uri.getPath();
        mImageFileWeakHashMap.put(id,path);
    }
    public WeakHashMap<Integer, String> getImageFiles(){
        return mImageFileWeakHashMap;
    }
    private void createNewImageView() {
        mTargetImageVew = new ImageView(getContext());
     //   mTargetImageVew.setBackgroundResource(R.drawable.border_text);
        mTargetImageVew.setPadding(1, 1, 1, 1);
        mTargetImageVew.setId(mCurrentNum);
        mTargetImageVew.setScaleType(ImageView.ScaleType.CENTER_CROP);
                   ...
                   // params.height = 55;
                    ...
                    window.findViewById(R.id.dialog_image_clicked_btn_delete)
                            .setOnClickListener(new OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    final ImageView delegateImageView = (ImageView) findViewById(mDeleteId);
                                    //设置图片逐渐消失的动画
                                    AlphaAnimation alphaAnimation = new AlphaAnimation(1, 0);
                                    alphaAnimation.setDuration(300);
                                    alphaAnimation.setRepeatCount(0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!mIsOnceInitOnMeasure) {
            mParams = new LayoutParams(imageSideLen, imageSideLen-70);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
       ...
       // mIconRemove.setLayoutParams(mParams);
